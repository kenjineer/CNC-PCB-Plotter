...
void draw()
{
  ...
  text(status, 12, y); y -= dy;
  text (done, 12, y); 
}

void init(){
  res = JOptionPane.showConfirmDialog(frame, "Are you sure you want to START PLOTTING\n             with the selected GCODE?", "Confirm Selection", JOptionPane.YES_NO_OPTION);
  if (res == JOptionPane.YES_OPTION){
      streaming = true;
      status = "Status: PLOTTING START";
  }
...
void halt(){
  status = "Status: PLOTTING STOPPED";
  String msg = status;
  JOptionPane.showMessageDialog(frame, msg, "Stop", JOptionPane.WARNING_MESSAGE);
  finish();
}

void finish(){
  status = "PCB IS READY FOR ETCHING AND CAN NOW BE REMOVED.";
  done = "Status: PLOTTING FINISHED";
  String msg = done + "\n" + status;
  JOptionPane.showMessageDialog(frame, msg, "Finish", JOptionPane.INFORMATION_MESSAGE);
}
